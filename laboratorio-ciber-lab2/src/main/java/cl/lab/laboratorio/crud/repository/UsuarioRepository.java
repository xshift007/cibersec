...
public interface UsuarioRepository extends JpaRepository<Usuario, Long> {
    ...
    @Query("SELECT new cl.lab.laboratorio.crud.dtos.UserData(u.sid, u.username, u.nombres, u.apellidos, u.correo) " +
            "FROM Usuario u " +
            "WHERE 1 = 1 " +
            "AND (:nombres IS NULL OR u.nombres = :nombres) " +
            "AND (:apellidos IS NULL OR u.apellidos = :apellidos) " +
            "AND (:username IS NULL OR u.username = :username )")
    List<UserData> findUsuariosByFilters(String nombres, String apellidos, String username);
